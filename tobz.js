...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6288806289497@c.us'
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Kamu belum cukup umur untuk menggunakan Gunsy BOT, min 16 tahun kamu bisa mendaftar ulang dengan cara Hubungi Owner : wa.me/6288806289497`, id) //if user is not registered
            }
        }
        ...
        const mess = {
            ...
            magernulissatu: 'Harap Tunggu, BOT Sedang Menulis Di Buku!',
            error: {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Gunsy!', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Gunsy!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
                            ...
                            if (found !== false) {
                                ...
                                tobz.sendText(from, resultv)
                                    ...
                                    tobz.reply(from, `Nomor tidak terdaftar didalam database!`, id)
                            }
                        })
                        return true;
                    }else{
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Gunsy!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Gunsy!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Gunsy!', id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink Gunsy*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'glitch':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev Gunsy*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev Gunsy*`, id)
            }
            break
        case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Umur terlalu tua kak, max 40 yaa :D', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                        ...
                        await tobz.reply(from, `Format yang kamu masukkan salah, kirim ${prefix}daftar |nama|umur\n\ncontoh format: ${prefix}daftar |aryo|17\n\ncukup gunakan nama depan/panggilan saja`, id) //if user is not registered
                    }
                break
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin Gunsy', id)  
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                            ...
                            tobz.sendTextWithMentions(from, result, id)
                                ...
                                tobz.reply(from, `${monospace(`Nomor tidak terdaftar didalam database!`)}`, id)
                        }
                break
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `6288806289497@c.us`)
            tobz.reply(from, 'Jika kalian ingin request fitur silahkan chat nomor owner kami!', id)
            break
        case prefix+'resetsticker':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Gunsy!`, id)
            if (!args.length === 1) return tobz.reply(from, `Masukkan nomornya, *GUNAKAN AWALAN 62*\ncontoh: #resetsticker 62852262236155 / #resetsticker @tagmember`, id) 
            const nomebr = args[1]
            ...
            const cuss = textz + '@c.us'
                ...
                if (found !== false) {
                    ...
                    limitAdd(serial)
                        ...
                        tobz.reply(from, `Maaf, Nomor tidak terdaftar di database!`, id)
                }
            break
        case prefix+'resetbadword':
            ...
            if(cekumur(cekage)) return
                    ...
                    if (!args.length === 1) return tobz.reply(from, 'Masukkan nomornya, *GUNAKAN AWALAN 62*\ncontoh: #resetbadword 6285112554122 / #resetbadword @tagmember') 
                    const nomer = args[1]
                    ...
                    const cus = text + '@c.us'
                        ...
                        if (found !== false) {
                            ...
                            limitAdd(serial)
                                ...
                                tobz.reply(from, `${monospace(`Maaf, Nomor tidak terdaftar di database!`)}`, id)
                        }
                break
        // ON OFF
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Gunsy!', id) // Hanya Admin yang bisa mengaktifkan
...
🌐️ *URL*: ${url}`
	...
	case prefix+'moddroid':
        ...
        case prefix+'video': // SEARCH VIDEO FROM YOUTUBE
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6288806289497', id)
            break
...
➸ *Cara:* ${cara}`
        ...
        case prefix+'twtstalk':
            ...
            console.log(twstalk)
            try {
            const twstalk2 = await axios.get('https://api.vhtear.com/twittprofile?username=' + twstalk + '&apikey=' + vhtearkey)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'nulis':
            ...
            const ngetikk = body.slice(7)
            const ngetikk = await axios.get('https://api.vhtear.com/write?text=' + ngetikk + '&apikey=' + vhtearkey)
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Gunsy!`, id)
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Gunsy`, id)
                bctxt = body.slice(4)
                txtbc = `*「 GUNSY BOT BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN Gunsy
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Gunsy', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Gunsy', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Gunsy', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Gunsy', id)
            await tobz.sendText(from,'Gunsy DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan Gunsy`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan Gunsy`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Gunsy atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'gunsyadmin':
            let admn = `This is list of Gunsy Admin\nTotal : ${adminNumber.length}\n`
        ...
        case prefix+'limit':
            ...
            break
        case prefix+'gift': // Hanya Admin & Owner Gunsy yang bisa gift Limit
            if (!isAdmin, !isOwner) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Gunsy!`, id)
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Gunsy!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Gunsy!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Gunsy!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Gunsy!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Gunsy!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Gunsy!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Gunsy!`, id)
        ...
        // LIST MENU
        case prefix+'runtime':
            tobz.reply(from, `Gunsy telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
            tobz.sendText(from, help(prefix, cts, pendaftar))
            break
        case prefix+'gunsygroup':
            tobz.reply(from, `Link Group Gunsy : https://chat.whatsapp.com/BBlDkf94XnL7QjeYlS7KJd\nJangan Lupa Join ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Gunsy', id)
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Gunsy!', id)
